...
import requests, os, sqlite3 # psycopg2
import datetime
...
# Conecta BBDD Local
bbdd_loc = working_dir + '/BD_Delta_001.sqlite'
...
Df_Estaciones =  pd.read_csv(working_dir + '/Estaciones/EstDelta_5347.csv')
...
def run():
    ...
    CB_Aguas_Arriba()
    CB_Frente_Margen_derecha()
    df_CB_F = CB_Frente_Margen_Izquierda()
    Temporal_Agrega_condBorde_Lujan(df_CB_F)
...
""" list_idSerie = [29,30,34]
obs = consultaAPI(f_inicio,f_fin,list_idSerie)
 """
...
def CB_Aguas_Arriba():
    ...
    df_todo = consultaAPI(f_inicio, f_fin,l_idEst)
...
def CB_Frente_Margen_derecha():
    print ('\nCB Frente Margen Derecha  ------------------')
    ### Consluta la BBDD
    l_idE_MDer = [52,1702,85]
    l_idS_MDer = [52,3308,85]
    ...
    # Consulta BBDD / Ver En Funciones
    df_todo = consultaAPI(f_inicio, f_fin,l_idS_MDer)

    for i in range(len(l_idE_MDer)):
        df_todo["id"].replace(l_idS_MDer[i],l_idE_MDer[i],inplace=True) 
...
def CB_Frente_Margen_Izquierda():
    ### Consluta la BBDD
    l_idE_MIzq = [1696,1699]
    l_idS_MIzq = [3278,3280]
    # Margen Derecha
    Df_EstacionesMI = Df_Estaciones[Df_Estaciones['unid'].isin(l_idE_MIzq)]
    print(Df_EstacionesMI[['unid','nombre']])

    # Consulta BBDD / Ver En Funciones
    df_todo = consultaAPI(f_inicio, f_fin,l_idS_MIzq)

    for i in range(len(l_idE_MIzq)):
        df_todo["id"].replace(l_idS_MIzq[i],l_idE_MIzq[i],inplace=True) 
    ...
    *   Calcula diferencias entre valores concecutivos.'''
    ...
    from Prono_Frente_api import pronofrente
    print('\n Pronostico  ---------------------------------')
...
def Temporal_Agrega_condBorde_Lujan(df_CB_F):
    print('\nTemporal:  ----------------------------------------')
