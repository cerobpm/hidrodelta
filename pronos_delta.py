...
def corrigeZarate(plots=False, upload=True, output_csv=False):
    ...
    df_Zarate_sim.index = df_Zarate_sim.index - timedelta(hours=2) # - timedelta(hours=2)
...
def corrigeAtucha(plots=False, upload=True, output_csv=False):
    ...
    ###### Correccion 1 el simulado en Atucha de retrasa dos horas
    df_Atucha_sim.index = df_Atucha_sim.index - timedelta(hours=2) # - timedelta(hours=2)
...
def corrigeRosario(plots=False,upload=True, output_csv=False):
    ...
    # PLOT FINAL
    plotFinal(df_Rosario_Obs,df_Rosario_sim2,nameout='productos/Prono_Rosario.png',ydisplay=-0.75,text_xoffset=(0.5,0),ylim=(-1,2), obsLine=False,nombre_estacion="Rosario", cero=2.92,fecha_emision=fecha_emision)
    ...
    # PLOT FINAL
    plotFinal(df_RosarioPNA_Obs,df_Rosario_sim2,nameout='productos/Prono_RosarioPNA.png',ydisplay=1.5,text_xoffset=(0,0),xytext=(-320,-200),ylim=(-1,2),obs_label='Nivel Observado PNA',extraObs=df_Rosario_Obs,extraObsLabel="Nivel Observado BDHI",fecha_emision=fecha_emision,obsLine=False,nombre_estacion="Rosario")
...
def corrigeVCons(plots=False,upload=True, output_csv=False):
    ...
    # PLOT FINAL
    plotFinal(df_VConstitucion_Obs,df_VConstitucion_sim2,nameout='productos/Prono_VConstitucion.png',ydisplay=-0.75,text_xoffset=(0.5,0),xytext=(-300,-100),ylim=(-1,2),bandaDeError=('e_pred_05','e_pred_95'),nombre_estacion="Villa Constitución",niveles_alerta={"aguas_bajas":1.9}, cero=1.98, fecha_emision=fecha_emision)
    ...
    # PLOT FINAL
    plotFinal(df_SanNicolas_Obs,df_SanNicolas_sim,nameout='productos/Prono_SanNicolas.png',ydisplay=-0.75,text_xoffset=(0.5,0.5),ylim=(-1.,2),bandaDeError=('e_pred_05','e_pred_95'),markersize=10,obsLine=False,nombre_estacion="San Nicolás",niveles_alerta={"aguas_bajas":1.8}, cero=1.91,fecha_emision=fecha_emision)
    ...
    ###### Correccion 1 el simulado en zarate de retrasa dos horas
    df_Ramallo_sim.index = df_Ramallo_sim.index - timedelta(hours=2)
    ...
    # 2 series
    series.append(prono2serie(df_Ramallo_sim,main_colname="Y_predic",members={'e_pred_01':'p01','e_pred_05':'p05','e_pred_95':'e95','e_pred_99':'p99'},series_id=3414))
    
    # PLOT FINAL
    plotFinal(df_Ramallo_Obs,df_Ramallo_sim,nameout='productos/Prono_Ramallo.png',ydisplay=-0.75,text_xoffset=(0.5,0.5),ylim=(-1.,2),bandaDeError=('e_pred_05','e_pred_95'),markersize=10,obsLine=False,nombre_estacion="Ramallo",niveles_alerta={"aguas_bajas":1.8}, cero=1.91,fecha_emision=fecha_emision)
...
def corrigeCarabelas(plots=False,upload=True,output_csv=False):
    ...
    ###### Correccion 1
    df_simulado.index = df_simulado.index - timedelta(hours=2) # + timedelta(hours=1) #
...
def corrigeNuevaPalmira(plots=False,upload=True,output_csv=False):
  ...
  f_inicio = datetime(2020,1, 1, 00, 0, 0, 0)
  f_fin = ahora
  ...
  print("Resample horario")
  df_Obs_H = df_Obs.resample('H').mean()
  df_Obs_H.index = df_Obs_H.index.tz_localize("America/Argentina/Buenos_Aires")
